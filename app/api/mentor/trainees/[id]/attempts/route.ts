...
export async function GET(
  ...
  try {
    ...
    // Verify this trainee belongs to the current mentor
    const trainee = await prisma.traineeProfile.findFirst({
      where: {
        id: traineeId,
        mentor: {
    ...
    if (!trainee) {
      return NextResponse.json(
        { error: "Trainee not found or unauthorized" },
        { status: 404 }
      );
    }
    ...
    console.error("Surgery attempts fetch error:", error);
    return NextResponse.json(
